...
import os
import cv2
from data.data import Read_MultipleDigit,data_process,Segment_variable,get_transform
...
def testv2(args):
    ...
    if args.variable_length:
        test_data,test_target,miniimg_list = data_process(handwrite_data,handwrite_target,handwrite=True,fixed_length=False)
    else:
        test_data,test_target = data_process(handwrite_data,handwrite_target,handwrite=True)
    if len(test_data)==0:
        return
    ...
    # 二次检测：
    if args.variable_length:
        predict_list = second_test(model,probability,predict_list,miniimg_list,False)
...
def run_experiment(args):
    ...
    if args.handwrite:
        ...
        if args.variable_length:
            test_data,test_targets,miniimg_list = data_process(test_data,test_targets,handwrite=False,fixed_length=False)
            if len(test_data)==0:
                return
...
if __name__ == '__main__':
    ...
    parser.add_argument('--variable_length',action='store_true',help='number of digits is variable')
