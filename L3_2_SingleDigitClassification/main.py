...
# train ResNet18
def train(model,train_dataloader,test_dataloader,args,use_cuda):
    criterion = torch.nn.CrossEntropyLoss() # loss函数：交叉熵损失，用于分类任务
    optimizer = torch.optim.SGD(model.parameters(),lr=args.learning_rate) # 优化器
    ...
    for e in tqdm(range(args.epoch)):
        ...
        # validate: 每5个epoch或最后一个epoch验证一次
        if (e+1)%5==0 or e==args.epoch-1:
            val_acc = predict(model,test_dataloader,use_cuda)
            # save model while acc value reach the highest.
            if val_acc > acc_temp:
                acc_temp = val_acc
                # save model
                torch.save(model,args.weight_path)
                print("Save Model in {} Successfully! Epoch:{} Acc:{} Loss:{}".format(args.weight_path,e,acc_temp,loss))
...
def run_experiment(args):
    ...
    # only test
    if args.test and os.path.exists(args.weight_path) and args.predict_file=='':
        if use_cuda:
            resnet18 = torch.load(args.weight_path) 
            resnet18.cuda()
        else :
            resnet18 = torch.load(args.weight_path,map_location='cpu') 
        predict(resnet18,test_dataloader,use_cuda)
    elif args.test and os.path.exists(args.weight_path) and os.path.exists(args.predict_file):
        resnet18 = torch.load(args.weight_path,map_location='cpu') 
        predictv2(resnet18,args.predict_file)
    elif args.test and ((not os.path.exists(args.weight_path)) or (not os.path.exists(args.predict_file))):
    ...
    # train
        ...
        if use_cuda:
            resnet18.cuda()
        train(resnet18,train_dataloader,test_dataloader,args,use_cuda) # 全参训练
        predict(resnet18,test_dataloader,use_cuda) # 测试
        

# 训练命令实例：
# python main.py --gpu 0 --batch_size 64 --dataset MNIST --data_path ./data/ --weight_path ./checkpoints/resnet18_ft.pth --epoch 20 --learning_rate 0.01 
if __name__ == '__main__':
    ...
    parser.add_argument('--weight_path',type=str,default='./checkpoints/resnet18_ft.pth', help='save model after training')
